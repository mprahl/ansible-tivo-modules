...
def get_season_episode_num(api_key, user_key, username, series_name,
    ...
    rv = requests_wrapper(
        ...
        error_msg=error_msg, fail_on_error=fail_on_error)
    ...
    rv = requests_wrapper(series_query_api_url, headers=headers,
                          error_msg=error_msg, fail_on_error=fail_on_error)
    if not rv:
        return None, None
    rv_json = get_request_json(rv, error_msg, fail_on_error)
    ...
    while True:
        ...
        for episode in rv_json['data']:
            if episode['episodeName'] == episode_name:
                return episode['dvdSeason'], episode['dvdEpisodeNumber']
...
def get_tivo_recording_info(hostname, mak, title, episode=None):
    ...
    """
    tivo_url = ('https://{0}/TiVoConnect?Command=QueryContainer&Container=%2F'
    ...
    rv = requests_wrapper(tivo_url, verify=False, error_msg=error_msg,
                          auth=HTTPDigestAuth('tivo', mak))

    # Strip out the namespace for convenience in queries later on
    xml_content = re.sub(r' xmlns="[^"]+"', '', rv.content, count=1)
    # Get an XML object
    ...
    for child in xml_root.iter('Item'):
        # If the "Item" tag doesn't have a child tag of "Details", then it's
        # not a tag with recording information
        child_details = child.find('Details')
        if child_details is None:
            continue
        # Check to see if the title of this section matches the desired title,
        # if not, go to the next recording
        current_title = child_details.find('Title').text
        if current_title != title:
            continue

        # At this point we have the right title (e.g. show name or movie name).
        # If the user specified an episode to download, then let's make sure
        # this is the correct episode.
        episode_element = child_details.find('EpisodeTitle')
        if episode_element is not None:
            # If the user didn't specify an episode or it's the desired episode
            if not episode or episode == episode_element.text:
                ...
                if episode:
                    # Since the user specified a single episode, that is all
                    # that needs to be returned so we can safely break
                    break
        elif episode_element is None and episode is None:
            # The user didn't specify an episode, and there is no episode value
            recordings.append({
                'title': title,
                'link': get_tivo_dl_link_from_element(child)
            })
            # We can break here since there are no recordings associated with
            # this title, so it'll be the only one on the Tivo
            break
...
def download_tivo_recording(mak, recording_info, dest_dir):
    ...
    tivo_recording_file = requests_wrapper(
        ...
        auth=HTTPDigestAuth('tivo', mak), timeout=None, stream=True)
...
def main():
    ...
    for recording_info in recordings_info:
        ...
        download_tivo_recording(module.params['mak'], recording_info,
